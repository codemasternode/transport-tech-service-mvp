...
exports.vehicleFilterByPallet = vehicleFilterByPallet;
exports.vehicleFilterByVolume = vehicleFilterByVolume;
...
function vehicleFilterByPallet(trucks, palettes, weight) {
  var w = weight / palettes; // To begin, we can carry nothing for no weight.

  var best_path = [{
    cost: 0
  }];
  trucks.forEach(function (truck) {
    var max_pallets = Math.floor(Math.min(truck.palettes, truck.capacity / w)); // i is the number of pallets other trucks carry.
...
function vehicleFilterByVolume(trucks, volume, weight) {
  volume = Math.ceil(volume);
  var w = weight / volume; // To begin, we can carry nothing for no weight.

  var best_path = [{
    cost: 0
  }];
  trucks.forEach(function (truck) {
    var max_volume = Math.floor(Math.min(truck.volume, truck.capacity / w)); // i is the number of volume other trucks carry.
    // We count down so that there is no chance the solution there
    // has already used this truck.
...
//# sourceMappingURL=vehicleFilter.js.map